...
const iconMap = [DollarSign, Clock, AlertTriangle] as const;

const ProblemSection = memo(() => {
  const { ref, isInView } = useInViewAnimation();

  // Memoize the header title to prevent re-renders
  ...
  // Memoize SVG circle calculations
  const circumference = useMemo(() => 2 * Math.PI * 50, []);
  const dashOffset = useMemo(() => circumference * (1 - 0.1181), [circumference]);
