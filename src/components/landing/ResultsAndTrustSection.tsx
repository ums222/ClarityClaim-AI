...
const trustBadges = ["HIPAA Compliant", "SOC 2 Type II", "HITRUST Certified", "99.99% Uptime"] as const;
const integrations = ["Epic", "Cerner", "Meditech", "Athena", "Change", "Availity"] as const;

// Memoized testimonial card component
const TestimonialCard = memo<{ testimonial: typeof testimonials[number] }>(({ testimonial }) => (
  <Card className="bg-slate-900/70 hover:-translate-y-1 hover:shadow-glow-primary transition-all">
    <CardHeader>
      <CardTitle className="text-sm">
        <span className="text-2xl leading-none text-clarity-accent">"</span>
        {testimonial.quote}
      </CardTitle>
    </CardHeader>
    <CardContent>
      <div className="mt-3 flex items-center gap-3">
        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-800 text-xs text-slate-300">
          {testimonial.author.split(" ")[1]?.[0] ?? "A"}
        </div>
        <div>
...
const ResultsAndTrustSection = memo(() => {
    ...
    <LazyMotion features={domAnimation} strict>
      <SectionContainer id="resources" className="bg-slate-950">
        <div ref={ref}>
          ...
          {/* Testimonials */}
          <div className="mt-12">
            <SectionHeader title="Trusted by Healthcare Leaders" align="center" />
            ...
            <div className="mt-4 flex gap-4 overflow-x-auto pb-3 md:hidden">
              {testimonials.map((t) => (
                <Card key={t.author} className="min-w-[260px] flex-1 bg-slate-900/80">
                  <CardHeader>
                    <CardTitle className="text-sm">
                      <span className="text-2xl leading-none text-clarity-accent">"</span>
                      {t.quote}
                  ...
                  <CardContent>
                    <div className="mt-3 flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-slate-50">{t.author}</p>
...
ResultsAndTrustSection.displayName = "ResultsAndTrustSection";
